...
//  @PreAuthorize("hasAuthority('ROLE_ADMIN')or hasAuthority('ROLE_USER')")
    ...
    @GetMapping("/serviceAccount/{serviceAccountId}")
//  @PreAuthorize("hasAuthority('ROLE_ADMIN')or hasAuthority('ROLE_USER')")
    public ResponseEntity<List<ProfileResponseDto>> getProfilesByServiceAccountId(@PathVariable Long serviceAccountId) {
        List<Profile> Profiles = profileService.getProfilesByServiceAccountId(serviceAccountId);
        return new ResponseEntity<>(mapper.toDtoList(Profiles), HttpStatus.OK);
    }
